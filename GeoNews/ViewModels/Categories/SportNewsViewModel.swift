...
class SportNewsViewModel {
    private var sportNewsItems: [News] = []
    
    var numberOfItems: Int { sportNewsItems.count }
    ...
    func news(at index: Int) -> News {
        return sportNewsItems[index]
    }
    
    func navigateToNewsDetails(index: Int) {
        self.onNewsSelected?(sportNewsItems[index])
    }
    ...
    private func getData() {
        NetworkService().fetchData(filterBy: "category", equalTo: "Sport") { [weak self] newsItems in
            ...
            if newsItems.isEmpty {
                ...
                self.sportNewsItems = newsItems.sorted {
