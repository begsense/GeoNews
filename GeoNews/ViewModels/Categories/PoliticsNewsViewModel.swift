...
class PoliticsNewsViewModel {
    private var politicNewsItems: [News] = []
    
    var numberOfItems: Int { politicNewsItems.count }
    ...
    func news(at index: Int) -> News {
        return politicNewsItems[index]
    }
    
    func navigateToNewsDetails(index: Int) {
        self.onNewsSelected?(politicNewsItems[index])
    }
    ...
    private func getData() {
        NetworkService().fetchData(filterBy: "category", equalTo: "Politics") { [weak self] newsItems in
            ...
            if newsItems.isEmpty {
                ...
                self.politicNewsItems = newsItems.sorted {
