...
class TechNewsViewModel {
    private var techNewsItems: [News] = []
    
    var numberOfItems: Int { techNewsItems.count }
    ...
    func news(at index: Int) -> News {
        return techNewsItems[index]
    }
    
    func navigateToNewsDetails(index: Int) {
        self.onNewsSelected?(techNewsItems[index])
    }
    ...
    func getData() {
        NetworkService().fetchData(filterBy: "category", equalTo: "Technology") { [weak self] newsItems in
            ...
            if newsItems.isEmpty {
                ...
                self.techNewsItems = newsItems.sorted {
